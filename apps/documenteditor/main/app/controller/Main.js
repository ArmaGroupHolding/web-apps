...
//                        this.onEditComplete(this.loadMask); //если делать фокус, то при принятии чужих изменений, заканчивается свой композитный ввод
            ...
            onValidateErrorPdfForm: function(oInfo) {
                ...
                if (oInfo["greater"] != null && oInfo["less"] != null) {
                    config.msg = `Ivalid value for field "${oInfo["target"].GetFullName()}": must be greater than or equal to ${oInfo["greater"]} \
                    ...
                    config.msg = `Ivalid value for field "${oInfo["target"].GetFullName()}": must be less than or equal to ${oInfo["less"]}`;
                }
